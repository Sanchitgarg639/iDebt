...
<html lang="en">
    ...
    <?php
        ...
        if((isset($_SESSION['lin']) && $_SESSION['lin'] == true) || (isset($_SESSION['sup']) && $_SESSION['sup'] == true)){
            ...
            echo "<h2 class='px-4 py-4'> Welcome ".$_SESSION['dname']."! Your customer's debts : </h2> ";
            ...
            $pagelas = $page+10;
            $query = "SELECT * FROM `debt` WHERE `dukan_number`=$dukan_number AND `id` BETWEEN ('$page') AND ('$pagelas') LIMIT 10";
            $result = mysqli_query($conn, $query);
            $rows = mysqli_num_rows($result);
            $no = 1;
            if($rows > 0){
                while($data = mysqli_fetch_assoc($result)){
                    ...
                    <div class='mx-4 lead py-2 my-4 displays'>";
                        ...
                        echo "<p class='deb'>Customer's Name: <b>".$data['debt_customer']."</b></p>";
                        if($data['debt_phone'] != ''){
            ...
            if((!isset($_GET['pg']) || $_GET['pg'] < 10) && $no != 10){
                echo '';
            }
            elseif(!isset($_GET['pg']) || $_GET['pg'] < 10){
                echo "<button class='btn btn-danger' disabled>Previous</button>
                <a href='trial.php?pg=10' class='btn btn-danger'>Next</a>";
            }
            elseif($no != 10){
                echo "<a href='trial.php?pg=1' class='btn btn-danger'>Previous</a>
        ...
        Array.from(deletes).forEach((element) => {
          element.addEventListener('click', (e)=>{
            ...
            if(confirm("Are you sure to Delete this Record!")){
              window.location = `http://localhost/php/idebt/debt.php?delete=true&id=${sno}`;
            }
          })
        })
    </script>
